...
async function loadMasterMaps() {
  ...
  if (await tableExists("units")) {
    ...
    const nameCol = cols.has("unit_name")
      ? "unit_name"
      : cols.has("symbol")
      ? "symbol"
      : cols.has("name")
  ...
  if (await tableExists("opening_stock")) {
    ...
    const rows = await queryAsync(`
      SELECT
        os.product_id,
        p.product_name,
        ${cols.has("product_type_id") ? "os.product_type_id" : "NULL AS product_type_id"},
        pt.product_type_en AS type_name,
        ${cols.has("category_id") ? "os.category_id" : "NULL AS category_id"},
        c.category_name,
        ${cols.has("quantity") ? "os.quantity" : "0 AS quantity"}
      FROM opening_stock os
      LEFT JOIN products p ON p.id = os.product_id
      LEFT JOIN product_types pt ON pt.id = os.product_type_id
